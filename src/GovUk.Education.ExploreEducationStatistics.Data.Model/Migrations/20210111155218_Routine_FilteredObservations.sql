CREATE OR ALTER PROCEDURE FilteredObservations
    ...
    @filterItemSearchTerms NVARCHAR(MAX)
AS
BEGIN
    DECLARE @timePeriodCount INT = (SELECT count(year) FROM @timePeriodList)
    DECLARE @observationalUnitExists BIT = CAST(IIF(
...
--     DECLARE c CURSOR FOR (SELECT year, timeIdentifier from @timePeriodList)
    DECLARE @sqlString NVARCHAR(MAX) = N'SELECT o.NaturalId ' +
                                       'FROM Observation o ' +
                                       'JOIN Location l ON o.LocationId = l.Id ' +
                                       'WHERE o.SubjectId = @subjectId '
    IF (@filterItemSearchTerms IS NOT NULL AND LEN(@filterItemSearchTerms) > 0)
        SET @sqlString = @sqlString + N'AND (' + @filterItemSearchTerms + ') '
